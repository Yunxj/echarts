<template>
    <div ref="water" class="water">
        <div class="full">
            <canvas ref="canva" class="w100 h100"></canvas>
            <input  ref="inputVal"
                :style="{display:'none'}"
                type="range"
                name="range"
                ...
                step="1"
            >
            <span class="unfinish"><span ref="dott">-----</span>{{total}}</span>
            <span class="finish"><span ref="dotted">--</span>{{finished}}</span>
        </div>
    </div>
</template>
<script>
export default {
    name: 'waterDraw',
    props: { color: { type: null, default: '#00cdfe' }, bg: { type: null, default: 'waterbg1' },total:{type:null},finished:{type:null} },
    data () {
        return {
            ow: '',
            oH: '',
    ...
    methods: {
        drawSine () {
            ...
            for (let i = this.xoffset; i <= this.xoffset + this.axisLength; i += 20 / this.axisLength) {
                let x = this.sp + (this.xoffset + i) / this.unit;
                let y = Math.sin(x) * this.nowrange;
                let dx = i;
                let dy = 2 * this.cRadius * (1 - this.nowdata) + (this.radius - this.cRadius) - (this.unit * y);
                ctx.lineTo(dx, dy);
                Stack.push([dx, dy])
            }
            // 获取初始点和结束点
        ...
        clipCircle () {
            ...
            let ctx = canvas.getContext("2d");
            ctx.beginPath();
            ctx.arc(this.radius, this.radius, this.cRadius, 0, 2 * Math.PI, false);
            ctx.clip();
        },
        render () {
            let canvas = this.$refs.canva;
    ...
    mounted () {
        this.$refs.dotted.style.color = this.color;
        this.$refs.dott.style.color = this.color;
        let canvas = this.$refs.canva;
        if (!canvas) return;
